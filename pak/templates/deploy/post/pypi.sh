#!/bin/bash
# Post-deployment template for pypi
...
echo "✅ Post-deployment tasks for pypi..."
...
jq --arg platform "pypi" --arg status "completed" --arg completed_at "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"    '.platforms[pypi] = {"status": , "completed_at": }'    "$PAK_DATA_DIR/deployments/$DEPLOY_ID.json" > temp.json && mv temp.json "$PAK_DATA_DIR/deployments/$DEPLOY_ID.json"

echo "✅ Post-deployment tasks completed for pypi"
