#!/bin/bash
# Post-deployment template for maven
...
echo "✅ Post-deployment tasks for maven..."
...
jq --arg platform "maven" --arg status "completed" --arg completed_at "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"    '.platforms[maven] = {"status": , "completed_at": }'    "$PAK_DATA_DIR/deployments/$DEPLOY_ID.json" > temp.json && mv temp.json "$PAK_DATA_DIR/deployments/$DEPLOY_ID.json"

echo "✅ Post-deployment tasks completed for maven"
